...
namespace asdm {
	...
	for (unsigned int i = 0; i <x.chanFreq.length(); ++i) {
		chanFreq.push_back(x.chanFreq[i]);
	}
	chanWidth .clear();
	for (unsigned int i = 0; i <x.chanWidth.length(); ++i) {
		chanWidth.push_back(x.chanWidth[i]);
	}
	refTemp .clear();
	...
	for (unsigned int i = 0; i < x.refTemp.length(); ++i) {
		...
		for (unsigned int j = 0; j < x.refTemp[0].length(); ++j) {
			v_aux_refTemp.push_back(x.refTemp[i][j]);
		}
	...
	for (unsigned int i = 0; i <x.polyFreqLimits.length(); ++i) {
		polyFreqLimits.push_back(x.polyFreqLimits[i]);
	}
	wetPath .clear();
	...
	setCalDataId(Parser::getTag("calDataId","CalWVR",rowDoc));
	setCalReductionId(Parser::getTag("calReductionId","CalWVR",rowDoc));
