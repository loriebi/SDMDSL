...
namespace asdm {
	...
	for (unsigned int i = 0; i <x.freqLimits.length(); ++i) {
		freqLimits.push_back(x.freqLimits[i]);
	}
	...
	setCalDataId(Parser::getTag("calDataId","CalBandpass",rowDoc));
	setCalReductionId(Parser::getTag("calReductionId","CalBandpass",rowDoc));
