...
namespace asdm {
	...
	for (unsigned int i = 0; i <x.focusPosition.length(); ++i) {
		focusPosition.push_back(x.focusPosition[i]);
	}
	...
	setBeamMapUID(x.beamMapUID);
	setRawRMS(x.rawRMS);
	setWeightedRMS(x.weightedRMS);
	setSurfaceMapUID(x.surfaceMapUID);
	direction .clear();
	...
	for (unsigned int i = 0; i <x.screwMotion.length(); ++i) {
		screwMotion.push_back(x.screwMotion[i]);
	}
	}
	...
	for (unsigned int i = 0; i <x.screwMotionError.length(); ++i) {
		screwMotionError.push_back(x.screwMotionError[i]);
	}
	}
	...
	for (unsigned int i = 0; i <x.gravOptRange.length(); ++i) {
		gravOptRange.push_back(x.gravOptRange[i]);
	}
	}
	...
	for (unsigned int i = 0; i <x.tempOptRange.length(); ++i) {
		tempOptRange.push_back(x.tempOptRange[i]);
	}
	}
	...
	setCalDataId(Parser::getTag("calDataId","CalHolography",rowDoc));
	setCalReductionId(Parser::getTag("calReductionId","CalHolography",rowDoc));
