...
,
	...
	* @param velDef
	...
	void DopplerTable::toIDL(asdmIDL::DopplerTableIDL& x) const {
		unsigned int nrow = size();
		x.row.length(nrow);
		vector<DopplerRow*> v = get();
		for (unsigned int i = 0; i < nrow; ++i) {
	...
	string DopplerTable::toXML()  {
		...
		buf.append("<DopplerTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:=\"http://Alma/XASDM/DopplerTable\" xsi:schemaLocation=\"http://Alma/XASDM/DopplerTable http://almaobservatory.org/XML/XASDM//DopplerTable.xsd\" schemaVersion=\"\" schemaRevision=\"-1\">\n");
	...
	string DopplerTable::getVersion() const {
		return version;
	}
	
	
	void DopplerTable::fromXML(string& tableInXML)  {
		//
		...
		if ( root_element == NULL || root_element->type != XML_ELEMENT_NODE )
			throw ConversionException("Failed to retrieve the root element in the DOM structure.", "Doppler");
	...
	string DopplerTable::MIMEXMLPart(const asdm::ByteOrder* byteOrder) {
		...
		oss << "\n";
		oss << "<DopplerTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:=\"http://Alma/XASDM/DopplerTable\" xsi:schemaLocation=\"http://Alma/XASDM/DopplerTable http://almaobservatory.org/XML/XASDM//DopplerTable.xsd\" schemaVersion=\"\" schemaRevision=\"-1\">\n";
	...
	void DopplerTable::setFromMIME(const string & mimeMsg) {
	   ...
	   //
		...
		if (getContainer().checkRowUniqueness()) {
	    	try {
	      		for (uint32_t i = 0; i < this->declaredSize; i++) {
					DopplerRow* aRow = DopplerRow::fromBin((EndianIStream&) eiss, *this, attributesSeq);
					checkAndAdd(aRow);
	      		}
	    	}
		...
		BinaryAttributeReaderFunctor* DopplerTable::getUnknownAttributeBinaryReader(const string& attributeName) const {
	...
	/* 
	  void DopplerTable::openMIMEFile (const string& directory) {
