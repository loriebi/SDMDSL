...
,
	...
	* @param spectralWindowId
	 ...
	 */ 
		...
		void DataDescriptionTable::addWithoutCheckingUnique(DataDescriptionRow * x) {
			if (getRowByKey(
			x->getDataDescriptionId()
			) != (DataDescriptionRow *) 0) 
				throw DuplicateKey("Dupicate key exception in ", "DataDescriptionTable");
			row.push_back(x);
			privateRows.push_back(x);
			x->isAdded(true);
		}
	...
	void DataDescriptionTable::toIDL(asdmIDL::DataDescriptionTableIDL& x) const {
		unsigned int nrow = size();
		x.row.length(nrow);
		vector<DataDescriptionRow*> v = get();
		for (unsigned int i = 0; i < nrow; ++i) {
...
#ifndef WITHOUT_ACS
	void DataDescriptionTable::fromIDL(DataDescriptionTableIDL x) {
	...
	string DataDescriptionTable::toXML()  {
		string buf;
	
		buf.append("<?xml version=\"1.0\" encoding=\"ISO-8859-1\"?> ");
		buf.append("<DataDescriptionTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:=\"http://Alma/XASDM/DataDescriptionTable\" xsi:schemaLocation=\"http://Alma/XASDM/DataDescriptionTable http://almaobservatory.org/XML/XASDM//DataDescriptionTable.xsd\" schemaVersion=\"\" schemaRevision=\"-1\">\n");
	...
	string DataDescriptionTable::getVersion() const {
		return version;
	}
	
	
	void DataDescriptionTable::fromXML(string& tableInXML)  {
		//
		...
		if ( doc == NULL )
			throw ConversionException("Failed to parse the xmlHeader into a DOM structure.", "DataDescription");
		
		xmlNode* root_element = xmlDocGetRootElement(doc);
		if ( root_element == NULL || root_element->type != XML_ELEMENT_NODE )
			throw ConversionException("Failed to retrieve the root element in the DOM structure.", "DataDescription");
		...
		if (getContainer().checkRowUniqueness()) {
			...
			try {
				...
				// cout << "Unexpected error in DataDescriptionTable::addWithoutCheckingUnique called from DataDescriptionTable::fromXML " << endl;
			}
		}				
	...
	void DataDescriptionTable::error()  {
		throw ConversionException("Invalid xml document","DataDescription");
	}
	
	string DataDescriptionTable::MIMEXMLPart(const asdm::ByteOrder* byteOrder) {
		...
		oss << "\n";
		oss << "<DataDescriptionTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:=\"http://Alma/XASDM/DataDescriptionTable\" xsi:schemaLocation=\"http://Alma/XASDM/DataDescriptionTable http://almaobservatory.org/XML/XASDM//DataDescriptionTable.xsd\" schemaVersion=\"\" schemaRevision=\"-1\">\n";
	...
	string DataDescriptionTable::toMIME(const asdm::ByteOrder* byteOrder) {
	...
	void DataDescriptionTable::setFromMIME(const string & mimeMsg) {
	    ...
	    if ( loc0 == string::npos) {
	      ...
	      if  ( loc0 == string::npos ) 
		      throw ConversionException("Failed to detect the beginning of the XML header", "DataDescription");
	    }
	    ...
	    if ( loc1 == string::npos ) {
	      throw ConversionException("Failed to detect the beginning of the binary part", "DataDescription");
	    }
	   ...
	   //
	    ...
	    attributesSeq.push_back("polOrHoloId") ; 
	    ...
	    version = "2";         
	    ...
	    else if (string("DataDescriptionTable").compare((const char*) root_element->name) == 0) {
	      ...
	      if ( bulkStoreRef == 0 || (bulkStoreRef->type != XML_ELEMENT_NODE)  || (string("BulkStoreRef").compare((const char*) bulkStoreRef->name) != 0))
	      	throw ConversionException ("Could not find the element '/DataDescriptionTable/BulkStoreRef'. Invalid XML header '"+ xmlHeader + "'.", "DataDescription");
	      ...
	      if (byteOrderAttr == 0) 
		     throw ConversionException("Could not find the element '/DataDescriptionTable/BulkStoreRef/@byteOrder'. Invalid XML header '" + xmlHeader +"'.", "DataDescription");
	      
	      string byteOrderValue = string((const char*) byteOrderAttr->children->content);
	      if (!(byteOrder = asdm::ByteOrder::fromString(byteOrderValue)))
			throw ConversionException("No valid value retrieved for the element '/DataDescriptionTable/BulkStoreRef/@byteOrder'. Invalid XML header '" + xmlHeader + "'.", "DataDescription");
	     ...
	     if ( attributes == 0 || (attributes->type != XML_ELEMENT_NODE)  || (string("Attributes").compare((const char*) attributes->name) != 0))	 
	       	throw ConversionException ("Could not find the element '/DataDescriptionTable/Attributes'. Invalid XML header '"+ xmlHeader + "'.", "DataDescription");
		...
		if (getContainer().checkRowUniqueness()) {
	    	try {
	      		for (uint32_t i = 0; i < this->declaredSize; i++) {
					DataDescriptionRow* aRow = DataDescriptionRow::fromBin((EndianIStream&) eiss, *this, attributesSeq);
					checkAndAdd(aRow);
	      		}
	    	}
	    	catch (DuplicateKey e) {
	      		throw ConversionException("Error while writing binary data , the message was "
					+ e.getMessage(), "DataDescription");
	    	}
		...
		BinaryAttributeReaderFunctor* DataDescriptionTable::getUnknownAttributeBinaryReader(const string& attributeName) const {
	...
	void DataDescriptionTable::setFromMIMEFile(const string& directory) {
	    string tablePath ;
	    
	    tablePath = directory + "/DataDescription.bin";
	    ifstream tablefile(tablePath.c_str(), ios::in|ios::binary);
	    if (!tablefile.is_open()) { 
	      throw ConversionException("Could not open file " + tablePath, "DataDescription");
	    }
	    // Read in a stringstream.
	    stringstream ss; ss << tablefile.rdbuf();
	    
	    if (tablefile.rdstate() == istream::failbit || tablefile.rdstate() == istream::badbit) {
	      throw ConversionException("Error reading file " + tablePath,"DataDescription");
	    }
	    
	    // And close.
	    tablefile.close();
	    if (tablefile.rdstate() == istream::failbit)
	      throw ConversionException("Could not close file " + tablePath,"DataDescription");
	    
	    setFromMIME(ss.str());
	  }	
	/* 
	  void DataDescriptionTable::openMIMEFile (const string& directory) {
	 ...
	 */
	void DataDescriptionTable::setFromXMLFile(const string& directory) {
	    string tablePath ;
	    
	    tablePath = directory + "/DataDescription.xml";
	    ...
	    */
	    ...
	    try {
	    	...
	    	throw ConversionException("Caugth an exception whose message is '" + e.getMessage() + "'.", "DataDescription");
	    }
