...
namespace asdm {
	...
	setGroundPressure(x.groundPressure);
	setGroundRelHumidity(x.groundRelHumidity);
	frequencySpectrum .clear();
	for (unsigned int i = 0; i <x.frequencySpectrum.length(); ++i) {
		frequencySpectrum.push_back(x.frequencySpectrum[i]);
	}
	setGroundTemperature(x.groundTemperature);
	...
	for (unsigned int i = 0; i < x.tAtmSpectrum.length(); ++i) {
		...
		for (unsigned int j = 0; j < x.tAtmSpectrum[0].length(); ++j) {
			v_aux_tAtmSpectrum.push_back(x.tAtmSpectrum[i][j]);
		}
	...
	for (unsigned int i = 0; i < x.tRecSpectrum.length(); ++i) {
		...
		for (unsigned int j = 0; j < x.tRecSpectrum[0].length(); ++j) {
			v_aux_tRecSpectrum.push_back(x.tRecSpectrum[i][j]);
		}
	...
	for (unsigned int i = 0; i < x.tSysSpectrum.length(); ++i) {
		...
		for (unsigned int j = 0; j < x.tSysSpectrum[0].length(); ++j) {
			v_aux_tSysSpectrum.push_back(x.tSysSpectrum[i][j]);
		}
	...
	for (unsigned int i = 0; i <x.water.length(); ++i) {
		water.push_back(x.water[i]);
	}
	waterError .clear();
	for (unsigned int i = 0; i <x.waterError.length(); ++i) {
		waterError.push_back(x.waterError[i]);
	}
	...
	setCalDataId(Parser::getTag("calDataId","CalAtmosphere",rowDoc));
	setCalReductionId(Parser::getTag("calReductionId","CalAtmosphere",rowDoc));
