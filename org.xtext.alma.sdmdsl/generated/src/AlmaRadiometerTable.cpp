...
,
 "numAntenna" , "spectralWindowId" 
	 ...
	 */ 
		...
		void AlmaRadiometerTable::addWithoutCheckingUnique(AlmaRadiometerRow * x) {
			if (getRowByKey(
			x->getAlmaRadiometerId()
			) != (AlmaRadiometerRow *) 0) 
				throw DuplicateKey("Dupicate key exception in ", "AlmaRadiometerTable");
			row.push_back(x);
			privateRows.push_back(x);
			x->isAdded(true);
		}
	...
	//
	void AlmaRadiometerTable::append(AlmaRadiometerRow *x) {
		privateRows.push_back(x);
		x->isAdded(true);
	}
	...
	void AlmaRadiometerTable::toIDL(asdmIDL::AlmaRadiometerTableIDL& x) const {
		unsigned int nrow = size();
		x.row.length(nrow);
		vector<AlmaRadiometerRow*> v = get();
		for (unsigned int i = 0; i < nrow; ++i) {
	...
	string AlmaRadiometerTable::toXML()  {
		...
		buf.append("<AlmaRadiometerTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:=\"http://Alma/XASDM/AlmaRadiometerTable\" xsi:schemaLocation=\"http://Alma/XASDM/AlmaRadiometerTable http://almaobservatory.org/XML/XASDM//AlmaRadiometerTable.xsd\" schemaVersion=\"\" schemaRevision=\"-1\">\n");
	...
	string AlmaRadiometerTable::getVersion() const {
		return version;
	}
	
	
	void AlmaRadiometerTable::fromXML(string& tableInXML)  {
		//
		...
		if ( root_element == NULL || root_element->type != XML_ELEMENT_NODE )
			throw ConversionException("Failed to retrieve the root element in the DOM structure.", "AlmaRadiometer");
		...
		if (getContainer().checkRowUniqueness()) {
			...
			try {
				...
				// cout << "Unexpected error in AlmaRadiometerTable::addWithoutCheckingUnique called from AlmaRadiometerTable::fromXML " << endl;
			}
		}				
	...
	string AlmaRadiometerTable::MIMEXMLPart(const asdm::ByteOrder* byteOrder) {
		...
		oss << "\n";
		oss << "<AlmaRadiometerTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:=\"http://Alma/XASDM/AlmaRadiometerTable\" xsi:schemaLocation=\"http://Alma/XASDM/AlmaRadiometerTable http://almaobservatory.org/XML/XASDM//AlmaRadiometerTable.xsd\" schemaVersion=\"\" schemaRevision=\"-1\">\n";
	...
	void AlmaRadiometerTable::setFromMIME(const string & mimeMsg) {
	   ...
	   //
		...
		if (getContainer().checkRowUniqueness()) {
	    	try {
	      		for (uint32_t i = 0; i < this->declaredSize; i++) {
					AlmaRadiometerRow* aRow = AlmaRadiometerRow::fromBin((EndianIStream&) eiss, *this, attributesSeq);
					checkAndAdd(aRow);
	      		}
	    	}
		...
		void AlmaRadiometerTable::setUnknownAttributeBinaryReader(const string& attributeName, BinaryAttributeReaderFunctor* barFctr) {
			//
		...
		BinaryAttributeReaderFunctor* AlmaRadiometerTable::getUnknownAttributeBinaryReader(const string& attributeName) const {
	...
	/* 
	  void AlmaRadiometerTable::openMIMEFile (const string& directory) {
	...
	void AlmaRadiometerTable::setFromXMLFile(const string& directory) {
	    ...
	    */
	    ...
	    try {
	    	...
	    	throw ConversionException("Caugth an exception whose message is '" + e.getMessage() + "'.", "AlmaRadiometer");
	    }
