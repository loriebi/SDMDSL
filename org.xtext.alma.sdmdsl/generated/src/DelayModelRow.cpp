...
namespace asdm {
	...
	for (unsigned int i = 0; i <x.LOOffset.length(); ++i) {
		LOOffset.push_back(x.LOOffset[i]);
	}
	}
	...
	for (unsigned int i = 0; i <x.LOOffsetRate.length(); ++i) {
		LOOffsetRate.push_back(x.LOOffsetRate[i]);
	}
	}
	...
	setAntennaId(Parser::getTag("antennaId","DelayModel",rowDoc));
	setFieldId(Parser::getTag("fieldId","DelayModel",rowDoc));
	setSpectralWindowId(Parser::getTag("spectralWindowId","DelayModel",rowDoc));
