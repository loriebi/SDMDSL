...
,
 "pressure" , "relHumidity" , "temperature" , "windDirection" , "windSpeed" , "windMax" , "dewPoint" , "numLayer" , "layerHeight" , "temperatureProfile" , "cloudMonitor" , "numWVR" , "wvrTemp" , "water" 
	...
	* @param timeInterval
	 */
	WeatherRow* WeatherTable::newRow(Tag stationId, ArrayTimeInterval timeInterval){
	...
	WeatherRow*  WeatherTable::checkAndAdd(WeatherRow* x, bool ) {
		string keystr = Key( 
		x->getStationId() 
							   ); 
	...
	void WeatherTable::toIDL(asdmIDL::WeatherTableIDL& x) const {
		unsigned int nrow = size();
		x.row.length(nrow);
		vector<WeatherRow*> v = get();
		for (unsigned int i = 0; i < nrow; ++i) {
	...
	string WeatherTable::toXML()  {
		...
		buf.append("<WeatherTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:=\"http://Alma/XASDM/WeatherTable\" xsi:schemaLocation=\"http://Alma/XASDM/WeatherTable http://almaobservatory.org/XML/XASDM//WeatherTable.xsd\" schemaVersion=\"\" schemaRevision=\"-1\">\n");
	...
	string WeatherTable::getVersion() const {
		return version;
	}
	
	
	void WeatherTable::fromXML(string& tableInXML)  {
		//
		...
		if ( root_element == NULL || root_element->type != XML_ELEMENT_NODE )
			throw ConversionException("Failed to retrieve the root element in the DOM structure.", "Weather");
	...
	string WeatherTable::MIMEXMLPart(const asdm::ByteOrder* byteOrder) {
		...
		oss << "\n";
		oss << "<WeatherTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:=\"http://Alma/XASDM/WeatherTable\" xsi:schemaLocation=\"http://Alma/XASDM/WeatherTable http://almaobservatory.org/XML/XASDM//WeatherTable.xsd\" schemaVersion=\"\" schemaRevision=\"-1\">\n";
	...
	void WeatherTable::setFromMIME(const string & mimeMsg) {
	   ...
	   //
	    ...
	    attributesSeq.push_back("numLayer") ; 
	    attributesSeq.push_back("layerHeight") ; 
	    attributesSeq.push_back("temperatureProfile") ; 
	    attributesSeq.push_back("cloudMonitor") ; 
	    attributesSeq.push_back("numWVR") ; 
	    attributesSeq.push_back("wvrTemp") ; 
		...
		if (getContainer().checkRowUniqueness()) {
	    	try {
	      		for (uint32_t i = 0; i < this->declaredSize; i++) {
					WeatherRow* aRow = WeatherRow::fromBin((EndianIStream&) eiss, *this, attributesSeq);
					checkAndAdd(aRow);
	      		}
	    	}
		...
		void WeatherTable::setUnknownAttributeBinaryReader(const string& attributeName, BinaryAttributeReaderFunctor* barFctr) {
			//
		...
		BinaryAttributeReaderFunctor* WeatherTable::getUnknownAttributeBinaryReader(const string& attributeName) const {
	...
	/* 
	  void WeatherTable::openMIMEFile (const string& directory) {
