...
,
	...
	* @param command
	...
	void FlagCmdTable::toIDL(asdmIDL::FlagCmdTableIDL& x) const {
		unsigned int nrow = size();
		x.row.length(nrow);
		vector<FlagCmdRow*> v = get();
		for (unsigned int i = 0; i < nrow; ++i) {
	...
	string FlagCmdTable::toXML()  {
		...
		buf.append("<FlagCmdTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:=\"http://Alma/XASDM/FlagCmdTable\" xsi:schemaLocation=\"http://Alma/XASDM/FlagCmdTable http://almaobservatory.org/XML/XASDM//FlagCmdTable.xsd\" schemaVersion=\"\" schemaRevision=\"-1\">\n");
	...
	string FlagCmdTable::getVersion() const {
		return version;
	}
	
	
	void FlagCmdTable::fromXML(string& tableInXML)  {
		//
		...
		if ( root_element == NULL || root_element->type != XML_ELEMENT_NODE )
			throw ConversionException("Failed to retrieve the root element in the DOM structure.", "FlagCmd");
	...
	string FlagCmdTable::MIMEXMLPart(const asdm::ByteOrder* byteOrder) {
		...
		oss << "\n";
		oss << "<FlagCmdTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:=\"http://Alma/XASDM/FlagCmdTable\" xsi:schemaLocation=\"http://Alma/XASDM/FlagCmdTable http://almaobservatory.org/XML/XASDM//FlagCmdTable.xsd\" schemaVersion=\"\" schemaRevision=\"-1\">\n";
	...
	void FlagCmdTable::setFromMIME(const string & mimeMsg) {
	   ...
	   //
		...
		BinaryAttributeReaderFunctor* FlagCmdTable::getUnknownAttributeBinaryReader(const string& attributeName) const {
