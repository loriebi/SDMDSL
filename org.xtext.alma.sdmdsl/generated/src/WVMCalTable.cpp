...
,
	...
	* @param inputAntennaId
	...
	WVMCalRow*  WVMCalTable::checkAndAdd(WVMCalRow* x, bool ) {
		string keystr = Key( 
		x->getAntennaId() ,
		x->getSpectralWindowId() 
							   ); 
	...
	//
	void WVMCalTable::append(WVMCalRow *x) {
	...
	string WVMCalTable::toXML()  {
		...
		buf.append("<WVMCalTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:=\"http://Alma/XASDM/WVMCalTable\" xsi:schemaLocation=\"http://Alma/XASDM/WVMCalTable http://almaobservatory.org/XML/XASDM//WVMCalTable.xsd\" schemaVersion=\"\" schemaRevision=\"-1\">\n");
	...
	string WVMCalTable::getVersion() const {
		return version;
	}
	
	
	void WVMCalTable::fromXML(string& tableInXML)  {
		//
		...
		if ( root_element == NULL || root_element->type != XML_ELEMENT_NODE )
			throw ConversionException("Failed to retrieve the root element in the DOM structure.", "WVMCal");
	...
	string WVMCalTable::MIMEXMLPart(const asdm::ByteOrder* byteOrder) {
		...
		oss << "\n";
		oss << "<WVMCalTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:=\"http://Alma/XASDM/WVMCalTable\" xsi:schemaLocation=\"http://Alma/XASDM/WVMCalTable http://almaobservatory.org/XML/XASDM//WVMCalTable.xsd\" schemaVersion=\"\" schemaRevision=\"-1\">\n";
	...
	void WVMCalTable::setFromMIME(const string & mimeMsg) {
	   ...
	   //
		...
		if (getContainer().checkRowUniqueness()) {
	    	try {
	      		for (uint32_t i = 0; i < this->declaredSize; i++) {
					WVMCalRow* aRow = WVMCalRow::fromBin((EndianIStream&) eiss, *this, attributesSeq);
					checkAndAdd(aRow);
	      		}
	    	}
		...
		void WVMCalTable::setUnknownAttributeBinaryReader(const string& attributeName, BinaryAttributeReaderFunctor* barFctr) {
			//
		...
		BinaryAttributeReaderFunctor* WVMCalTable::getUnknownAttributeBinaryReader(const string& attributeName) const {
	...
	/* 
	  void WVMCalTable::openMIMEFile (const string& directory) {
