...
namespace asdm {
	...
	setExecBlockId(x.execBlockId);
	setStartTime(x.startTime);
	setEndTime(x.endTime);
	setExecBlockNum(x.execBlockNum);
	setExecBlockUID(x.execBlockUID);
	setProjectUID(x.projectUID);
	...
	setSessionReference(x.sessionReference);
	setBaseRangeMin(x.baseRangeMin);
	setBaseRangeMax(x.baseRangeMax);
	setBaseRmsMinor(x.baseRmsMinor);
	setBaseRmsMajor(x.baseRmsMajor);
	setBasePa(x.basePa);
	setAborted(x.aborted);
	setNumAntenna(x.numAntenna);
	releaseDateExists = x.releaseDateExists;
	if (x.releaseDateExists) {
	setReleaseDate(x.releaseDate);
	}
