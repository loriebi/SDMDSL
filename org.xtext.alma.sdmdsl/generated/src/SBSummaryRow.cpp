...
namespace asdm {
	...
	setSbSummaryUID(x.sbSummaryUID);
	setProjectUID(x.projectUID);
	setObsUnitSetUID(x.obsUnitSetUID);
	setFrequency(x.frequency);
	setFrequencyBand(x.frequencyBand);
	setSbType(x.sbType);
	setSbDuration(x.sbDuration);
	...
	for (unsigned int i = 0; i <x.centerDirection.length(); ++i) {
		centerDirection.push_back(x.centerDirection[i]);
	}
	}
	...
	setCenterDirectionEquinox(x.centerDirectionEquinox);
	}
