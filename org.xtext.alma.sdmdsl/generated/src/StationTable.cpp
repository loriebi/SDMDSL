 ...
 "time" 
	...
	* @param type
	...
	string StationTable::toXML()  {
		...
		buf.append("<StationTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:=\"http://Alma/XASDM/StationTable\" xsi:schemaLocation=\"http://Alma/XASDM/StationTable http://almaobservatory.org/XML/XASDM//StationTable.xsd\" schemaVersion=\"\" schemaRevision=\"-1\">\n");
	...
	string StationTable::getVersion() const {
		return version;
	}
	
	
	void StationTable::fromXML(string& tableInXML)  {
		//
		...
		if ( root_element == NULL || root_element->type != XML_ELEMENT_NODE )
			throw ConversionException("Failed to retrieve the root element in the DOM structure.", "Station");
	...
	string StationTable::MIMEXMLPart(const asdm::ByteOrder* byteOrder) {
		...
		oss << "\n";
		oss << "<StationTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:=\"http://Alma/XASDM/StationTable\" xsi:schemaLocation=\"http://Alma/XASDM/StationTable http://almaobservatory.org/XML/XASDM//StationTable.xsd\" schemaVersion=\"\" schemaRevision=\"-1\">\n";
	...
	void StationTable::setFromMIME(const string & mimeMsg) {
	   ...
	   //
		...
		if (getContainer().checkRowUniqueness()) {
	    	try {
	      		for (uint32_t i = 0; i < this->declaredSize; i++) {
					StationRow* aRow = StationRow::fromBin((EndianIStream&) eiss, *this, attributesSeq);
					checkAndAdd(aRow);
	      		}
	    	}
		...
		BinaryAttributeReaderFunctor* StationTable::getUnknownAttributeBinaryReader(const string& attributeName) const {
	...
	/* 
	  void StationTable::openMIMEFile (const string& directory) {
