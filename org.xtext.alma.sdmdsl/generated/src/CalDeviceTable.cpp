...
,
 "numReceptor" , "calEff" , "noiseCal" , "coupledNoiseCal" , "temperatureLoad" 
	...
	CalDeviceTable::CalDeviceTable(ASDM &c) : container(c) {
		...
		entity.setEntityTypeName("CalDeviceTable");
	 ...
	 */
	ASDM &CalDeviceTable::getContainer() const {
		return container;
	}
	 ...
	 */
	Entity CalDeviceTable::getEntity() const {
		return entity;
	}
	 ...
	 */
	void CalDeviceTable::setEntity(Entity e) {
		this->entity = e; 
	}
	 ...
	 * Create a new row.
	 */
	CalDeviceRow *CalDeviceTable::newRow() {
	...
	* @param calLoadNames
	...
	CalDeviceRow* CalDeviceTable::add(CalDeviceRow* x) {
	...
	CalDeviceRow*  CalDeviceTable::checkAndAdd(CalDeviceRow* x, bool ) {
		...
		vector<CalDeviceRow*>& found = context.find(keystr)->second;
	...
	CalDeviceTableIDL *CalDeviceTable::toIDL() {
		CalDeviceTableIDL *x = new CalDeviceTableIDL ();
		unsigned int nrow = size();
		x->row.length(nrow);
		vector<CalDeviceRow*> v = get();
	...
	void CalDeviceTable::toIDL(asdmIDL::CalDeviceTableIDL& x) const {
		unsigned int nrow = size();
		x.row.length(nrow);
		vector<CalDeviceRow*> v = get();
		for (unsigned int i = 0; i < nrow; ++i) {
	...
	string CalDeviceTable::toXML()  {
		...
		buf.append("<CalDeviceTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:=\"http://Alma/XASDM/CalDeviceTable\" xsi:schemaLocation=\"http://Alma/XASDM/CalDeviceTable http://almaobservatory.org/XML/XASDM//CalDeviceTable.xsd\" schemaVersion=\"\" schemaRevision=\"-1\">\n");
	...
	string CalDeviceTable::getVersion() const {
		return version;
	}
	
	
	void CalDeviceTable::fromXML(string& tableInXML)  {
		//
		...
		if ( root_element == NULL || root_element->type != XML_ELEMENT_NODE )
			throw ConversionException("Failed to retrieve the root element in the DOM structure.", "CalDevice");
		...
		if (!xml.isStr("<CalDeviceTable")) 
			error();
		// cout << "Parsing a CalDeviceTable" << endl;
		string s = xml.getElement("<Entity","/>");
		if (s.length() == 0) 
			error();
		Entity e;
		e.setFromXML(s);
		if (e.getEntityTypeName() != "CalDeviceTable")
			error();
		setEntity(e);
	...
	string CalDeviceTable::MIMEXMLPart(const asdm::ByteOrder* byteOrder) {
		...
		oss << "\n";
		oss << "<CalDeviceTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:=\"http://Alma/XASDM/CalDeviceTable\" xsi:schemaLocation=\"http://Alma/XASDM/CalDeviceTable http://almaobservatory.org/XML/XASDM//CalDeviceTable.xsd\" schemaVersion=\"\" schemaRevision=\"-1\">\n";
	...
	void CalDeviceTable::setFromMIME(const string & mimeMsg) {
	   ...
	   //
		...
		if (getContainer().checkRowUniqueness()) {
	    	try {
	      		for (uint32_t i = 0; i < this->declaredSize; i++) {
					CalDeviceRow* aRow = CalDeviceRow::fromBin((EndianIStream&) eiss, *this, attributesSeq);
					checkAndAdd(aRow);
	      		}
	    	}
		...
		void CalDeviceTable::setUnknownAttributeBinaryReader(const string& attributeName, BinaryAttributeReaderFunctor* barFctr) {
			//
		...
		BinaryAttributeReaderFunctor* CalDeviceTable::getUnknownAttributeBinaryReader(const string& attributeName) const {
	...
	/* 
	  void CalDeviceTable::openMIMEFile (const string& directory) {
	...
	/**
	 * Insert a CalDeviceRow* in a vector of CalDeviceRow* so that it's ordered by ascending start time.
	 *
	 * @param CalDeviceRow* x . The pointer to be inserted.
	 * @param vector <CalDeviceRow*>& row. A reference to the vector where to insert x.
	 *
	 */
	 CalDeviceRow* CalDeviceTable::insertByStartTime(CalDeviceRow* x, vector<CalDeviceRow*>& row) {
	 				
	 		vector <CalDeviceRow*>::iterator theIterator;
