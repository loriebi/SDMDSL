...
namespace asdm {
	...
	for (unsigned int i = 0; i < x.frequencyRange.length(); ++i) {
		...
		for (unsigned int j = 0; j < x.frequencyRange[0].length(); ++j) {
			v_aux_frequencyRange.push_back(x.frequencyRange[i][j]);
		}
	...
	setBeamDescriptionUID(x.beamDescriptionUID);
	...
	for (unsigned int i = 0; i <x.minValidDirection.length(); ++i) {
		minValidDirection.push_back(x.minValidDirection[i]);
	}
	maxValidDirection .clear();
	for (unsigned int i = 0; i <x.maxValidDirection.length(); ++i) {
		maxValidDirection.push_back(x.maxValidDirection[i]);
	}
	...
	for (unsigned int i = 0; i <x.imageNominalFrequency.length(); ++i) {
		imageNominalFrequency.push_back(x.imageNominalFrequency[i]);
	}
	...
	setCalDataId(Parser::getTag("calDataId","CalPrimaryBeam",rowDoc));
	setCalReductionId(Parser::getTag("calReductionId","CalPrimaryBeam",rowDoc));
