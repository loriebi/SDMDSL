...
namespace asdm {
	...
	for (unsigned int i = 0; i <x.temperatureLoad.length(); ++i) {
		temperatureLoad.push_back(x.temperatureLoad[i]);
	}
	}
	...
	setAntennaId(Parser::getTag("antennaId","CalDevice",rowDoc));
	setFeedId(Parser::getInteger("feedId","CalDevice",rowDoc));
	setSpectralWindowId(Parser::getTag("spectralWindowId","CalDevice",rowDoc));
