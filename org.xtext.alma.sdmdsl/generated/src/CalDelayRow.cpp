...
namespace asdm {
	...
	for (unsigned int i = 0; i <x.refFreqPhase.length(); ++i) {
		refFreqPhase.push_back(x.refFreqPhase[i]);
	}
	}
	...
	setCalDataId(Parser::getTag("calDataId","CalDelay",rowDoc));
	setCalReductionId(Parser::getTag("calReductionId","CalDelay",rowDoc));
