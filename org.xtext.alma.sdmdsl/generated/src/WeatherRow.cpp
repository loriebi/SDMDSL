...
namespace asdm {
	...
	for (unsigned int i = 0; i <x.layerHeight.length(); ++i) {
		layerHeight.push_back(x.layerHeight[i]);
	}
	}
	...
	for (unsigned int i = 0; i <x.temperatureProfile.length(); ++i) {
		temperatureProfile.push_back(x.temperatureProfile[i]);
	}
	}
	cloudMonitorExists = x.cloudMonitorExists;
	if (x.cloudMonitorExists) {
	setCloudMonitor(x.cloudMonitor);
	}
	...
	setStationId(Parser::getTag("stationId","Weather",rowDoc));
