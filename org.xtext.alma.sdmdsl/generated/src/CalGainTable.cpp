...
,
	...
	* @param totalFitWeight
	 */
	CalGainRow* CalGainTable::newRow(Tag calDataId, Tag calReductionId, ArrayTime startValidTime, ArrayTime endValidTime, float gain, bool gainValid, float fit, float fitWeight, bool totalGainValid, float totalFit, float totalFitWeight){
	...
	//
	void CalGainTable::append(CalGainRow *x) {
		privateRows.push_back(x);
		x->isAdded(true);
	}
...
#ifndef WITHOUT_ACS
	void CalGainTable::fromIDL(CalGainTableIDL x) {
	...
	string CalGainTable::toXML()  {
		string buf;
	
		buf.append("<?xml version=\"1.0\" encoding=\"ISO-8859-1\"?> ");
		buf.append("<CalGainTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:=\"http://Alma/XASDM/CalGainTable\" xsi:schemaLocation=\"http://Alma/XASDM/CalGainTable http://almaobservatory.org/XML/XASDM//CalGainTable.xsd\" schemaVersion=\"\" schemaRevision=\"-1\">\n");
	...
	string CalGainTable::getVersion() const {
		return version;
	}
	
	
	void CalGainTable::fromXML(string& tableInXML)  {
		//
		...
		if ( doc == NULL )
			throw ConversionException("Failed to parse the xmlHeader into a DOM structure.", "CalGain");
		
		xmlNode* root_element = xmlDocGetRootElement(doc);
		if ( root_element == NULL || root_element->type != XML_ELEMENT_NODE )
			throw ConversionException("Failed to retrieve the root element in the DOM structure.", "CalGain");
	...
	void CalGainTable::error()  {
		throw ConversionException("Invalid xml document","CalGain");
	}
	
	string CalGainTable::MIMEXMLPart(const asdm::ByteOrder* byteOrder) {
		...
		oss << "\n";
		oss << "<CalGainTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:=\"http://Alma/XASDM/CalGainTable\" xsi:schemaLocation=\"http://Alma/XASDM/CalGainTable http://almaobservatory.org/XML/XASDM//CalGainTable.xsd\" schemaVersion=\"\" schemaRevision=\"-1\">\n";
	...
	string CalGainTable::toMIME(const asdm::ByteOrder* byteOrder) {
	...
	void CalGainTable::setFromMIME(const string & mimeMsg) {
	    ...
	    if ( loc1 == string::npos ) {
	      throw ConversionException("Failed to detect the beginning of the binary part", "CalGain");
	    }
	   ...
	   //
	    ...
	    attributesSeq.push_back("gain") ; 
	    attributesSeq.push_back("gainValid") ; 
	    attributesSeq.push_back("fit") ; 
	    attributesSeq.push_back("fitWeight") ; 
	    attributesSeq.push_back("totalGainValid") ; 
	    attributesSeq.push_back("totalFit") ; 
	    attributesSeq.push_back("totalFitWeight") ; 
	    
	    
	    // And decide that it has version == "2"
	    version = "2";         
	     }
	    else if (string("CalGainTable").compare((const char*) root_element->name) == 0) {
	      ...
	      if ( bulkStoreRef == 0 || (bulkStoreRef->type != XML_ELEMENT_NODE)  || (string("BulkStoreRef").compare((const char*) bulkStoreRef->name) != 0))
	      	throw ConversionException ("Could not find the element '/CalGainTable/BulkStoreRef'. Invalid XML header '"+ xmlHeader + "'.", "CalGain");
	      ...
	      if (byteOrderAttr == 0) 
		     throw ConversionException("Could not find the element '/CalGainTable/BulkStoreRef/@byteOrder'. Invalid XML header '" + xmlHeader +"'.", "CalGain");
	      
	      string byteOrderValue = string((const char*) byteOrderAttr->children->content);
	      if (!(byteOrder = asdm::ByteOrder::fromString(byteOrderValue)))
			throw ConversionException("No valid value retrieved for the element '/CalGainTable/BulkStoreRef/@byteOrder'. Invalid XML header '" + xmlHeader + "'.", "CalGain");
	     ...
	     if ( attributes == 0 || (attributes->type != XML_ELEMENT_NODE)  || (string("Attributes").compare((const char*) attributes->name) != 0))	 
	       	throw ConversionException ("Could not find the element '/CalGainTable/Attributes'. Invalid XML header '"+ xmlHeader + "'.", "CalGain");
		...
		if (getContainer().checkRowUniqueness()) {
	    	try {
	      		for (uint32_t i = 0; i < this->declaredSize; i++) {
					CalGainRow* aRow = CalGainRow::fromBin((EndianIStream&) eiss, *this, attributesSeq);
					checkAndAdd(aRow);
	      		}
	    	}
	    	catch (DuplicateKey e) {
	      		throw ConversionException("Error while writing binary data , the message was "
					+ e.getMessage(), "CalGain");
	    	}
		...
		void CalGainTable::setUnknownAttributeBinaryReader(const string& attributeName, BinaryAttributeReaderFunctor* barFctr) {
			//
		...
		BinaryAttributeReaderFunctor* CalGainTable::getUnknownAttributeBinaryReader(const string& attributeName) const {
	...
	void CalGainTable::setFromMIMEFile(const string& directory) {
	    string tablePath ;
	    
	    tablePath = directory + "/CalGain.bin";
	    ifstream tablefile(tablePath.c_str(), ios::in|ios::binary);
	    if (!tablefile.is_open()) { 
	      throw ConversionException("Could not open file " + tablePath, "CalGain");
	    }
	    // Read in a stringstream.
	    stringstream ss; ss << tablefile.rdbuf();
	    
	    if (tablefile.rdstate() == istream::failbit || tablefile.rdstate() == istream::badbit) {
	      throw ConversionException("Error reading file " + tablePath,"CalGain");
	    }
	    
	    // And close.
	    tablefile.close();
	    if (tablefile.rdstate() == istream::failbit)
	      throw ConversionException("Could not close file " + tablePath,"CalGain");
	    
	    setFromMIME(ss.str());
	  }	
	/* 
	  void CalGainTable::openMIMEFile (const string& directory) {
	 ...
	 */
	void CalGainTable::setFromXMLFile(const string& directory) {
	    string tablePath ;
	    
	    tablePath = directory + "/CalGain.xml";
