...
namespace asdm {
	...
	for (unsigned int i = 0; i < x.tcalSpectrum.length(); ++i) {
		...
		for (unsigned int j = 0; j < x.tcalSpectrum[0].length(); ++j) {
			v_aux_tcalSpectrum.push_back(x.tcalSpectrum[i][j]);
		}
	...
	for (unsigned int i = 0; i < x.trxSpectrum.length(); ++i) {
		...
		for (unsigned int j = 0; j < x.trxSpectrum[0].length(); ++j) {
			v_aux_trxSpectrum.push_back(x.trxSpectrum[i][j]);
		}
	...
	for (unsigned int i = 0; i < x.tskySpectrum.length(); ++i) {
		...
		for (unsigned int j = 0; j < x.tskySpectrum[0].length(); ++j) {
			v_aux_tskySpectrum.push_back(x.tskySpectrum[i][j]);
		}
	...
	setAntennaId(Parser::getTag("antennaId","SysCal",rowDoc));
	setFeedId(Parser::getInteger("feedId","SysCal",rowDoc));
	setSpectralWindowId(Parser::getTag("spectralWindowId","SysCal",rowDoc));
