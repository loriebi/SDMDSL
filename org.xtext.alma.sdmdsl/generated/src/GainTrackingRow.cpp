...
namespace asdm {
	...
	for (unsigned int i = 0; i <x.attSpectrum.length(); ++i) {
		attSpectrum.push_back(x.attSpectrum[i]);
	}
	}
	...
	setAntennaId(Parser::getTag("antennaId","GainTracking",rowDoc));
	setFeedId(Parser::getInteger("feedId","GainTracking",rowDoc));
	setSpectralWindowId(Parser::getTag("spectralWindowId","GainTracking",rowDoc));
