...
,
 "measuredFocusPosition" , "measuredFocusRotation" 
	...
	* @param focusModelId
	 */
	FocusRow* FocusTable::newRow(Tag antennaId, ArrayTimeInterval timeInterval, bool focusTracking, vector<Length > focusOffset, vector<Angle > focusRotationOffset, int focusModelId){
		...
		row->setFocusRotationOffset(focusRotationOffset);
	...
	//
	void FocusTable::append(FocusRow *x) {
	...
	void FocusTable::toIDL(asdmIDL::FocusTableIDL& x) const {
		unsigned int nrow = size();
		x.row.length(nrow);
		vector<FocusRow*> v = get();
		for (unsigned int i = 0; i < nrow; ++i) {
	...
	string FocusTable::toXML()  {
		...
		buf.append("<FocusTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:=\"http://Alma/XASDM/FocusTable\" xsi:schemaLocation=\"http://Alma/XASDM/FocusTable http://almaobservatory.org/XML/XASDM//FocusTable.xsd\" schemaVersion=\"\" schemaRevision=\"-1\">\n");
	...
	string FocusTable::getVersion() const {
		return version;
	}
	
	
	void FocusTable::fromXML(string& tableInXML)  {
		//
		...
		if ( root_element == NULL || root_element->type != XML_ELEMENT_NODE )
			throw ConversionException("Failed to retrieve the root element in the DOM structure.", "Focus");
	...
	string FocusTable::MIMEXMLPart(const asdm::ByteOrder* byteOrder) {
		...
		oss << "\n";
		oss << "<FocusTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:=\"http://Alma/XASDM/FocusTable\" xsi:schemaLocation=\"http://Alma/XASDM/FocusTable http://almaobservatory.org/XML/XASDM//FocusTable.xsd\" schemaVersion=\"\" schemaRevision=\"-1\">\n";
		...
		oss << "<Attributes>\n";
		oss << "<antennaId/>\n"; 	oss << "<timeInterval/>\n"; 	oss << "<focusTracking/>\n"; 	oss << "<focusOffset/>\n"; 	oss << "<focusRotationOffset/>\n"; 	oss << "<focusModelId/>\n"; 
		oss << "<measuredFocusPosition/>\n"; 	oss << "<measuredFocusRotation/>\n"; 
		oss << "</Attributes>\n";		
		oss << "</FocusTable>\n";
	...
	void FocusTable::setFromMIME(const string & mimeMsg) {
	   ...
	   //
	    ...
	    attributesSeq.push_back("focusRotationOffset") ; 
	    attributesSeq.push_back("focusModelId") ; 
	    attributesSeq.push_back("measuredFocusPosition") ; 
	    attributesSeq.push_back("measuredFocusRotation") ; 
		...
		if (getContainer().checkRowUniqueness()) {
	    	try {
	      		for (uint32_t i = 0; i < this->declaredSize; i++) {
					FocusRow* aRow = FocusRow::fromBin((EndianIStream&) eiss, *this, attributesSeq);
					checkAndAdd(aRow);
	      		}
	    	}
		...
		BinaryAttributeReaderFunctor* FocusTable::getUnknownAttributeBinaryReader(const string& attributeName) const {
	...
	/* 
	  void FocusTable::openMIMEFile (const string& directory) {
