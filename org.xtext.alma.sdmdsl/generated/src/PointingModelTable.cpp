 ...
 "coeffFormula" 
	...
	* @param assocPointingModelId
	...
	string PointingModelTable::toXML()  {
		...
		buf.append("<PointingModelTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:=\"http://Alma/XASDM/PointingModelTable\" xsi:schemaLocation=\"http://Alma/XASDM/PointingModelTable http://almaobservatory.org/XML/XASDM//PointingModelTable.xsd\" schemaVersion=\"\" schemaRevision=\"-1\">\n");
	...
	string PointingModelTable::getVersion() const {
		return version;
	}
	
	
	void PointingModelTable::fromXML(string& tableInXML)  {
		//
		...
		if ( root_element == NULL || root_element->type != XML_ELEMENT_NODE )
			throw ConversionException("Failed to retrieve the root element in the DOM structure.", "PointingModel");
	...
	string PointingModelTable::MIMEXMLPart(const asdm::ByteOrder* byteOrder) {
		...
		oss << "\n";
		oss << "<PointingModelTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:=\"http://Alma/XASDM/PointingModelTable\" xsi:schemaLocation=\"http://Alma/XASDM/PointingModelTable http://almaobservatory.org/XML/XASDM//PointingModelTable.xsd\" schemaVersion=\"\" schemaRevision=\"-1\">\n";
	...
	void PointingModelTable::setFromMIME(const string & mimeMsg) {
	   ...
	   //
		...
		if (getContainer().checkRowUniqueness()) {
	    	try {
	      		for (uint32_t i = 0; i < this->declaredSize; i++) {
					PointingModelRow* aRow = PointingModelRow::fromBin((EndianIStream&) eiss, *this, attributesSeq);
					checkAndAdd(aRow);
	      		}
	    	}
		...
		BinaryAttributeReaderFunctor* PointingModelTable::getUnknownAttributeBinaryReader(const string& attributeName) const {
	...
	/* 
	  void PointingModelTable::openMIMEFile (const string& directory) {
