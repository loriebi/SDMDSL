 ...
 "numBaseline" , "rms" 
	...
	* @param reducedChiSquared
	...
	CalBandpassRow* CalBandpassTable::add(CalBandpassRow* x) {
		...
		void CalBandpassTable::addWithoutCheckingUnique(CalBandpassRow * x) {
			if (getRowByKey(
			x->getBasebandName(),
			x->getSideband(),
			x->getAtmPhaseCorrection(),
			...
			x->getCalReductionId()
				...
				throw DuplicateKey("Dupicate key exception in ", "CalBandpassTable");
			row.push_back(x);
			privateRows.push_back(x);
			x->isAdded(true);
		}
	...
	//
	void CalBandpassTable::append(CalBandpassRow *x) {
		privateRows.push_back(x);
		x->isAdded(true);
	}
	...
	string CalBandpassTable::toXML()  {
		...
		buf.append("<CalBandpassTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:=\"http://Alma/XASDM/CalBandpassTable\" xsi:schemaLocation=\"http://Alma/XASDM/CalBandpassTable http://almaobservatory.org/XML/XASDM//CalBandpassTable.xsd\" schemaVersion=\"\" schemaRevision=\"-1\">\n");
	...
	string CalBandpassTable::getVersion() const {
		return version;
	}
	
	
	void CalBandpassTable::fromXML(string& tableInXML)  {
		//
		...
		if ( root_element == NULL || root_element->type != XML_ELEMENT_NODE )
			throw ConversionException("Failed to retrieve the root element in the DOM structure.", "CalBandpass");
		...
		if (getContainer().checkRowUniqueness()) {
			...
			try {
				...
				// cout << "Unexpected error in CalBandpassTable::addWithoutCheckingUnique called from CalBandpassTable::fromXML " << endl;
			}
		}				
	...
	string CalBandpassTable::MIMEXMLPart(const asdm::ByteOrder* byteOrder) {
		...
		oss << "\n";
		oss << "<CalBandpassTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:=\"http://Alma/XASDM/CalBandpassTable\" xsi:schemaLocation=\"http://Alma/XASDM/CalBandpassTable http://almaobservatory.org/XML/XASDM//CalBandpassTable.xsd\" schemaVersion=\"\" schemaRevision=\"-1\">\n";
	...
	void CalBandpassTable::setFromMIME(const string & mimeMsg) {
	   ...
	   //
		...
		BinaryAttributeReaderFunctor* CalBandpassTable::getUnknownAttributeBinaryReader(const string& attributeName) const {
	...
	void CalBandpassTable::setFromXMLFile(const string& directory) {
	    ...
	    */
	    ...
	    try {
	    	...
	    	throw ConversionException("Caugth an exception whose message is '" + e.getMessage() + "'.", "CalBandpass");
	    }
