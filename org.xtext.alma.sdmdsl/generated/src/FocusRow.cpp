...
namespace asdm {
	...
	for (unsigned int i = 0; i <x.focusOffset.length(); ++i) {
		focusOffset.push_back(x.focusOffset[i]);
	}
	focusRotationOffset .clear();
	for (unsigned int i = 0; i <x.focusRotationOffset.length(); ++i) {
		focusRotationOffset.push_back(x.focusRotationOffset[i]);
	}
	...
	for (unsigned int i = 0; i <x.measuredFocusPosition.length(); ++i) {
		measuredFocusPosition.push_back(x.measuredFocusPosition[i]);
	}
	}
	...
	for (unsigned int i = 0; i <x.measuredFocusRotation.length(); ++i) {
		measuredFocusRotation.push_back(x.measuredFocusRotation[i]);
	}
	}
	...
	setAntennaId(Parser::getTag("antennaId","Focus",rowDoc));
