...
from BinaryDataFlagsMod import *
from ACSErrTypeCommon import BadParameterEx

def fromString(stringVersionOfEnumeration):
	try:
		return {
			'INTEGRATION_FULLY_BLANKED': INTEGRATION_FULLY_BLANKED,
			'WVR_APC': WVR_APC,
			'CORRELATOR_MISSING_STATUS': CORRELATOR_MISSING_STATUS,
			'MISSING_ANTENNA_EVENT': MISSING_ANTENNA_EVENT,
			'DELTA_SIGMA_OVERFLOW': DELTA_SIGMA_OVERFLOW,
			'DELAY_CORRECTION_NOT_APPLIED': DELAY_CORRECTION_NOT_APPLIED,
			'SYNCRONIZATION_ERROR': SYNCRONIZATION_ERROR,
			'FFT_OVERFLOW': FFT_OVERFLOW,
			'TFB_SCALING_FACTOR_NOT_RETRIEVED': TFB_SCALING_FACTOR_NOT_RETRIEVED,
			'ZERO_LAG_NOT_RECEIVED': ZERO_LAG_NOT_RECEIVED,
			'SIGMA_OVERFLOW': SIGMA_OVERFLOW,
			'UNUSABLE_CAI_OUTPUT': UNUSABLE_CAI_OUTPUT,
			'QC_FAILED': QC_FAILED,
			'NOISY_TDM_CHANNELS': NOISY_TDM_CHANNELS,
			'SPECTRAL_NORMALIZATION_FAILED': SPECTRAL_NORMALIZATION_FAILED,
			'DROPPED_PACKETS': DROPPED_PACKETS,
			'DETECTOR_SATURATED': DETECTOR_SATURATED,
			'NO_DATA_FROM_DIGITAL_POWER_METER': NO_DATA_FROM_DIGITAL_POWER_METER,
